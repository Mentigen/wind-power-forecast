...
1. **LightGBM** - primary model, `objective=regression_l1` (MAE loss). Hyperparameters tuned via Optuna (25 trials). Trained with 5 seeds (42-46), predictions averaged to reduce variance. Fully deterministic: `deterministic=True`, `force_row_wise=True`, `num_threads=1`.
...
4. **Ensemble** - best option among all single models and fixed-weight combinations, selected by validation MAE. Equal weight (50/50 LGBM-avg + XGB) preferred when margin over weighted is below noise threshold.
...
│   ├── lgbm_model.pkl      # seed=42 alias
│   ├── lgbm_seed42.pkl     # individual seed models
│   ├── lgbm_seed43.pkl
│   ├── lgbm_seed44.pkl
│   ├── lgbm_seed45.pkl
│   ├── lgbm_seed46.pkl
